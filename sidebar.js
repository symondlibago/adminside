...
const CustomDrawerContent = () => {
  ...
  const DrawerItem = ({ icon, label, screenName }) => (
    ...
    >
      <Ionicons name={icon} size={24} color={screenName === selectedItem ? 'black' : 'white'} style={styles.drawerIcon} />
      <Text style={[styles.drawerItemText, { color: screenName === selectedItem ? 'black' : 'white' }]}>{label}</Text>
    </TouchableOpacity>
  );
